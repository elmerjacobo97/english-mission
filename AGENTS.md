...
Feature work is planned in `specs/NN-slug.md` (spec-driven flow, config in `specs/.spec-config.yml`). `/spec-impl` creates the branch `spec-NN-slug` automatically. Specs 01 (spaced repetition), 02 (daily streak) and 03 (coin shop) are implemented — read the one you touch before changing its behavior. Closing is local-only: `/spec-close` commits and merges on `main`; `main` is ahead of `origin/main` and is never pushed.
...
- `src/features/shop` — recharge mini-quiz behind `/shop`: max 3 paid recharges per local day (`src/lib/progress/shop.ts`), level-1 profile payout through `coinsForAttempt`.
